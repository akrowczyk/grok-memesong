...
function App() {
    ...
    const handleGenerate = async () => {
        ...
        try {
            // Step 1: Extract text from image using Tesseract (FREE)
            ...
            if (!cleanedText || cleanedText.length < 10) {
                throw new Error('Could not extract enough text from the image. Try a clearer screenshot.');
            }

            // Step 2: Generate song using Grok text model (CHEAP)
            setLoadingMessage('🎵 Writing your banger...');
            const result = await generateSong(apiKey, cleanedText, selectedPreset);
    ...
    const canGenerate = apiKey && image && selectedPreset && !isLoading;
